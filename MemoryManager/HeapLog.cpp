...
namespace FSMM {
    ...
    void HeapLog::Remove( void * block ) {
        ...
        if ( allocatedBlocks.find( block ) == allocatedBlocks.end() ) {
            // !!! You tried to delete an block of memory that ALREADY deleted or not exists. !!!
            // Use call stack to debug your shit!
            __debugbreak();
        }
    ...
    void HeapLog::WriteLogFile( const char * heapName ) {
        ...
        std::tr2::sys::path leakLogPath = std::tr2::sys::path( file_name.c_str() );
        ...
        OutputDebugStringA( ("Report: " + file_name + '\n' ).c_str() );
        OutputDebugString( L"!!! --> Memory Leaks Detected <-- !!!\n" );
