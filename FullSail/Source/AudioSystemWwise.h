...
public:
	...
	virtual bool RegisterEntity(const IEntity* entity, const char* name);
	virtual bool UnRegisterEntity(const IEntity* entity);
	virtual bool RegisterListener(const IEntity* entity, const char* name);
	virtual bool UnRegisterListener(const IEntity* entity);
	
	//Posts an audio event at the caller's position
	virtual bool PostEvent(AudioEvent eventId,const IEntity* eventCaller);
	...
	// Materials
	void SetMaterialID(const IEntity* pEntity, AkUniqueID akMaterialSwitchDefine, const unsigned int nMaterialID);
	...
	void SetRTCPValue(const char* szRTCPName, const AkRtpcValue akRtcpVal, const IEntity* pEntity);
	void SetRTCPValue(const AkRtpcID akRtcpID, const AkRtpcValue akRtcpVal);
	void SetRTCPValue(const AkRtpcID akRtcpID, const AkRtpcValue akRtcpVal, const IEntity* pEntity);
	...
	// Switches
	void SetSwitch(const AkSwitchGroupID akSwitchGroupID, const AkSwitchStateID akStateID, const IEntity* pEntity = nullptr);

	// Triggers
	void PostTrigger(const char* szTriggerName);
	void PostTrigger(const AkTriggerID akTriggerID);
	void PostTrigger(const char* szTriggerName, const IEntity* pEntity);
	void PostTrigger(const AkTriggerID akTriggerID, const IEntity* pEntity);
...
protected:
	AudioSystemWwise& operator=(const AudioSystemWwise&) = delete;

	static AudioSystemWwise*	m_spAudioSystem;

	std::vector<const IEntity*> m_RegisteredListeners;		// entities registered as Listeners
	std::vector<const IEntity*> m_RegisteredEntities;			// entities registered as game objects
