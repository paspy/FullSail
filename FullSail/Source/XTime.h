...
// XTime is a timer class designed to be used by D3D11 graphics applications.(use one per thread)
// Use it for tracking time intervals in seconds with double precision.
// It also supports weighted time smoothing for time based movement. (should not be used for tracking time)
// Future versions may support uploading time data across multiple threads. (data sent to separate thread profiler)
...
public:
    ...
    // This method can be used to preterit's a more granular version of the delta.(Better for motion)
    // This is achieved by using previous Delta values to compute a weighted running average.
    double SmoothDelta();
